...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/21 21:16:08 by tle-monn          #+#    #+#             */
/*   Updated: 2024/02/26 14:12:07 by tle-monn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	current_index(t_stack_node *stack)
{
	...
	while (stack) //Loop through all the nodes until the end of the stack
	{
		stack->index = i;
...
static void	set_target_a(t_stack_node *a, t_stack_node *b) //find `a` node's target in stack `b`
{
	...
	long			best_match_index; //the best match index stores the value of the "closest smaller number"=
...
static void	cost_analysis_a(t_stack_node *a, t_stack_node *b) //function that analyses the cost of the `a` node along with it's target `b` node, which is the sum of the number of instructions for both the nodes to rotate to the top of their stacks
{
	int	len_a;
	int	len_b;
...
void	init_nodes_a(t_stack_node *a, t_stack_node *b) //function that combines all the functions needed to prepare stack `a`, ready for our pushing and sorting. These functions set the data inside the node's structure
{
	current_index(a);
	current_index(b);
	set_target_a(a, b);
